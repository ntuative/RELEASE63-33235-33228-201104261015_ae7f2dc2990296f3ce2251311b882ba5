package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_1102:Number = 1.0;
      
      private var var_593:BitmapData;
      
      private var var_717:ColorTransform;
      
      private var var_1375:ColorTransform;
      
      private var var_1687:Matrix;
      
      private var _translationMatrix:Matrix;
      
      private var var_836:Number = 1.0;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_717 = new ColorTransform();
         this.var_717.alphaMultiplier = 1;
         this.var_1375 = new ColorTransform();
         this.var_1687 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_97 = this.var_1101[param1];
         this.var_904 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1686 >= 0)
         {
            this.var_246 = this._visualization.getSprite(this.var_1686);
            if(this.var_246 && this.var_246.asset)
            {
               ...
               if(this.var_83 == null)
               {
                  this.var_83 = this.var_246.asset.clone();
                  if(this.var_717.alphaMultiplier != 1)
                  {
                     this.var_593 = new BitmapData(this.var_83.width,this.var_83.height,true,4278190080);
                  }
               }
               this.var_2305 = -this.var_246.offsetX;
               this.var_2306 = -this.var_246.offsetY;
               this.var_246.asset = this.var_83;
            }
            if(this.var_83)
            {
            ...
            if(this.var_593)
            {
               this.var_593.fillRect(this.var_593.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_97.y * this.var_1102;
         }
         return 0;
      }
      ...
      {
         if(!this.var_97 || !this.var_246)
         {
            return;
         }
         if(this.var_83 && this.var_246.asset != this.var_83)
         {
            this.var_246.asset = this.var_83;
         }
         if(this.var_904)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_904 && this.var_97.hasIgnited)
         {
            this.var_904 = true;
         }
         _loc8_ = this.var_1202 * this.var_1102;
         this.var_97.update();
         if(this.var_904)
         {
            if(this.var_97.roomObjectSpriteId >= 0)
            ...
            {
               this.var_83.draw(this.var_593,this.var_1687,this.var_717,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_97.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2305 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1102;
               _loc4_ = this.var_2306 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1102;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1375.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_83.draw(_loc7_,this._translationMatrix,this.var_1375,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2304 = parseInt(param1.@size);
         this.var_1686 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1202 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1102 = this.var_2304 / 64;
         this.var_836 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_836 = Math.min(this.var_836,1);
         this.var_717.alphaMultiplier = this.var_836;
         for each(_loc3_ in param1.emitter)
         {
            ...
            for each(_loc23_ in _loc3_.particles.particle)
            {
               ...
               _loc19_ = _loc23_.@is_emitter == "HabboRoomObjectLogicLib_manifest" ? false : true;
               _loc20_ = _loc23_.hasOwnProperty("@fade") && _loc23_.@fade == "HabboAvatarRenderLib_HabboAvatarPetActions" ? true : false;
               _loc21_ = [];
