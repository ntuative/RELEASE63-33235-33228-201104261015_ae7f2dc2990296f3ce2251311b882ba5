package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      {
         if(this.var_944 >= this.getPageCount())
         {
            this.var_944 = Math.max(0,this.getPageCount() - 1);
         }
      }
      
      public function getPageCount() : int
      {
         return Math.ceil(this.var_224.length / const_438);
      }
      
      public function getStartFriendIndex() : int
      {
         this.checkPageIndex();
         return this.var_944 * const_438;
      }
      
      public function getEndFriendIndex() : int
      {
         this.checkPageIndex();
         return Math.min((this.var_944 + 1) * const_438,this.var_224.length);
      }
