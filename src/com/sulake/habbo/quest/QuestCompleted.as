package com.sulake.habbo.quest
{
   ...
   public class QuestCompleted implements IDisposable
   {
      ...
      private function onNextQuest(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this._window.visible = false;
         if(this.var_363.completedQuestsInCampaign >= this.var_363.questCountInCampaign)
         {
            this.var_363.id = 0;
            this.var_363.accepted = false;
            this._questEngine.controller.campaignCompleted.show(this.var_363);
         }
         else
         {
            this._questEngine.controller.questDetails.openForNextQuest = this._questEngine.configuration.getKey("questing.showDetailsForNextQuest") == "HabboAvatarRenderLib_HabboAvatarPetActions";
      ...
      public function prepare(param1:QuestMessageData) : void
      {
         ...
         if(this._window == null)
         {
            ...
            this._window.findChildByName("catalog_link_region").procedure = this.onRewardInfo;
         }
         this._window.findChildByName("catalog_link_txt").caption = this._questEngine.localization.getKey("quests.completed.cataloglink." + this.var_363.activityPointType);
         var _loc2_:String = "quests.completed.reward." + this.var_363.activityPointType;
         this._questEngine.localization.registerParameter(_loc2_,"amount",this.var_363.rewardCurrencyAmount.toString());
         this._window.findChildByName("reward_txt").caption = this._questEngine.localization.getKey(_loc2_,_loc2_);
         var _loc3_:* = this.var_363.getQuestLocalizationKey() + ".completed";
      ...
      public function onRewardInfo(param1:WindowEvent, param2:IWindow = null) : void
      {
         ...
         var _loc3_:String = "catalog.page.quest." + this.var_363.campaignCode;
