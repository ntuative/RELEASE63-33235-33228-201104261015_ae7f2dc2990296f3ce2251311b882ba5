package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_678:Rectangle;
      
      private var var_2023:int = 0;
      
      private var var_452:IAvatarRenderManager;
      
      private var var_1069:Boolean;
      
      private var var_2022:BitmapData;
      
      private const const_2111:int = 13421772;
      
      private const const_2112:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_832 = param3;
         this._window = param1;
         this.var_1414 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         this.var_2024 = param5;
         if(param3 == null)
         {
            this.var_701 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_2023 = Math.max(this.var_2023,_loc6_.colorLayerIndex);
            }
         }
         this.var_452 = this.var_66.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_701 != null && !this.var_2024)
            {
               _loc3_ = this.var_701;
            }
            else
            ...
            if(this.var_832)
            {
               switch(this.var_832.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_45:
                     _loc2_.style = HabboIconType.const_857;
                     break;
                  case HabboClubLevelEnum.const_46:
                     _loc2_.style = HabboIconType.const_688;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1414.color = this.const_2111;
         }
         else
         {
            this.var_1414.color = this.const_2112;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_452.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_452.isFigureReady(_loc1_))
         {
            this.var_452.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_368[_loc6_] + "_" + FigureData.const_135;
               _loc4_ = this.var_452.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_678 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_2022)
               {
                  ...
                  this.var_2022 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_678.width,this.var_678.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_678.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_678.y;
               if(this.var_2024 && _loc2_.colorLayerIndex > 0)
               {
