package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridColorItem
   {
      ...
      private var var_1005:IPartColor;
      ...
      private function updateThumbData() : void
      {
         ...
         if(_loc1_ != null)
         {
            _loc1_.bitmap = new BitmapData(this.var_603.width,this.var_603.height,true,0);
            _loc1_.bitmap.copyPixels(this.var_603,this.var_603.rect,new Point(0,0));
         }
         var _loc2_:IBitmapWrapperWindow = this._window.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
         if(_loc2_ != null)
         {
            _loc2_.bitmap = new BitmapData(this.var_923.width,this.var_923.height,true,0);
            _loc5_ = this.var_923.clone();
            _loc5_.colorTransform(_loc5_.rect,this.var_1005.colorTransform);
         ...
         if(_loc3_ != null)
         {
            if(this._isSelected)
            {
               _loc3_.bitmap = new BitmapData(this.var_925.width,this.var_925.height,true,16777215);
               _loc3_.bitmap.copyPixels(this.var_925,this.var_925.rect,new Point(0,0),null,null,true);
               _loc3_.visible = true;
            }
            else
            {
         ...
         if(_loc4_)
         {
            if(this.var_1005)
            {
               switch(this.var_1005.clubLevel)
               {
                  case HabboClubLevelEnum.const_53:
                     _loc4_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_45:
                     _loc4_.style = HabboIconType.const_857;
                     _loc4_.visible = true;
                     break;
                  case HabboClubLevelEnum.const_46:
                     _loc4_.style = HabboIconType.const_688;
                     _loc4_.visible = true;
               }
            }
            else
