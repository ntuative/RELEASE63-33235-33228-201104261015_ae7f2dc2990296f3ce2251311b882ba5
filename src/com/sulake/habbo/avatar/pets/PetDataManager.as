package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager, IDisposable
   {
      ...
      {
         var _loc1_:* = null;
         while(this.var_487.length > 0)
         {
            _loc1_ = this.var_487.pop() as IPetDataListener;
            ...
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.var_326.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         for each(_loc6_ in _loc4_)
         {
            ...
            {
               _loc7_ = false;
               if(this.var_594.getValue(_loc6_) == null)
               {
                  ...
                  this.var_594.add(_loc6_,_loc8_);
                  if(param3 != null && this.var_487.indexOf(param3) == -1)
                  {
                     this.var_487.push(param3);
                  }
               }
            }
         }
      ...
      private function unregisterLoadingAssets(param1:LibraryLoader) : void
      {
         ...
         for each(_loc3_ in this.var_594.getKeys())
         {
            _loc5_ = this.var_594.getValue(_loc3_);
            if(param1 == _loc5_)
            {
               _loc2_.push(_loc3_);
            }
