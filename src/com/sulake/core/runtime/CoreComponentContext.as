package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1395:String = "asset-libraries";
      
      private static const const_1390:String = "library";
      
      private static const const_1393:String = "service-libraries";
      
      private static const const_1396:String = "library";
      
      private static const const_1397:String = "component-libraries";
      
      private static const const_1394:String = "library";
      
      private static const const_1391:String = "error_data";
      
      private static const const_1398:String = "error_cat";
      
      private static const const_1392:String = "error_desc";
       
      
      private var var_354:LibraryLoaderQueue;
      
      private var var_771:IEventDispatcher;
      
      private var var_514:uint;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            case Core.const_1302:
               ...
               this.var_355 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_934:
               debug("Core; using complex frame update handler");
               this.var_355 = this.complexFrameUpdateHandler;
               break;
            case Core.const_932:
               debug("Core; using profiler frame update handler");
               this.var_355 = this.profilerFrameUpdateHandler;
               this.var_203 = new Profiler(this);
               attachComponent(this.var_203,[new IIDProfiler()]);
               var_401.addChild(new ProfilerViewer(this.var_203));
               break;
            case Core.const_629:
               debug("Core; using experimental frame update handler");
               this.var_355 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_355 = this.debugFrameUpdateHandler;
         }
      }
      
      public function getNumberOfFilesPending() : uint
      {
         return this.var_354.length;
      }
      
      public function getNumberOfFilesLoaded() : uint
      {
         return this.var_514 - this.getNumberOfFilesPending();
      }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1391,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1398,String(param3));
         ErrorReportStorage.setParameter(const_1392,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_771 = loadingEventDelegate;
         if(this.var_354 == null)
         {
            this.var_354 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1395)[0];
            if(node != null)
            {
               list = node.child(const_1390);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_320,e);
         }
         try
         {
            node = config.child(const_1393)[0];
            if(node != null)
            {
               list = node.child(const_1396);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_320,e);
         }
         try
         {
            node = config.child(const_1397)[0];
            if(node != null)
            {
               list = node.child(const_1394);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_320,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1694);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_771)
         {
            this.var_771.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_771)
         {
            ...
            this.var_771.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_771)
                  {
                     this.var_771.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_514 - this.var_354.length,this.var_514,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_203 ? 0 : this.var_1460 & 0;
         if(_loc3_ == Core.const_629)
         {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_355(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_108)
         {
            ...
            {
               if(this.var_632[priority] < priority)
               {
                  ++this.var_632[priority];
                  skip = true;
               }
            }
            if(!skip)
